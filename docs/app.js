const DATA_URL = 'data/westminster-shorter-catechism_pca.json';
const DB_NAME = 'confessio-study';
const state = { questions: [], progress: {}, selected: new Set(), history: [], session: null, charts: [] };
...
async function persist() {
  ...
  await dbSet('userData', { version: 1, progress: state.progress, selected: [...state.selected], history: state.history, exportedAt: new Date().toISOString() });
  $('save-label').textContent = 'Saved'; setTimeout(() => $('save-label').textContent = 'Ready', 1200);
}
function statusOf(number) { return state.progress[number]?.status || 'unlearned'; }
...
function renderLibrary() {
  const c = counts(), total = state.questions.length, percent = total ? Math.round(c.learned / total * 100) : 0;
  $('total-count').textContent = total; $('learned-count').textContent = c.learned; $('revisit-count').textContent = c.revisit; $('selected-count').textContent = state.selected.size;
  $('document-percent').textContent = `${percent}%`; $('document-progress-fill').style.width = `${percent}%`;
  $('selected-pool-label').textContent = `${state.selected.size} question${state.selected.size === 1 ? '' : 's'}`;
  $('unlearned-pool-label').textContent = `${c.unlearned} questions`; $('revisit-pool-label').textContent = `${c.revisit} questions`;
  const visible = visibleQuestions();
  $('select-all-checkbox').checked = visible.length > 0 && visible.every(q => state.selected.has(q.number));
  $('question-list').innerHTML = visible.length ? visible.map(q => `<label class="question-row"><input type="checkbox" data-question="${q.number}" ${state.selected.has(q.number) ? 'checked' : ''}><span class="number">Q${q.number}</span><span class="question">${q.question}<span class="answer-preview">${cleanAnswer(q.answer)}</span></span><span class="status-pill ${statusOf(q.number)}">${statusOf(q.number) === 'unlearned' ? 'Not started' : statusOf(q.number)}</span></label>`).join('') : '<p class="empty-state">No questions match this filter.</p>';
  document.querySelectorAll('[data-question]').forEach(input => input.addEventListener('change', () => { input.checked ? state.selected.add(+input.dataset.question) : state.selected.delete(+input.dataset.question); persist(); renderLibrary(); }));
}
function route(name) {
...
function startSession() {
  ...
  let pool = poolName === 'selected' ? state.questions.filter(q => state.selected.has(q.number)) : poolName === 'all' ? [...state.questions] : state.questions.filter(q => statusOf(q.number) === poolName);
...
async function recordAndNext(status, correct) { const s = state.session, q = s.pool[s.index]; state.progress[q.number] = { status, updatedAt: new Date().toISOString() }; state.history.push({ question: q.number, status, correct, method: s.method, at: new Date().toISOString() }); await persist(); s.index++; s.answered = false; if (s.index >= s.pool.length) finishSession(); else renderSession(); }
...
function renderAnalytics() {
  const c = counts(), learned = c.learned, reviewed = state.history.length, tests = state.history.filter(h => h.method !== 'flashcard'), correct = tests.filter(h => h.correct).length;
  ...
  const days = [...new Set(state.history.map(h => h.at.slice(0, 10)))].sort().reverse(); let streak = 0, cursor = new Date(); while (days.includes(cursor.toISOString().slice(0, 10))) { streak++; cursor.setDate(cursor.getDate() - 1); } $('analytics-streak').textContent = streak;
  ...
  const labels = [], activity = []; for (let i = 13; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); const key = d.toISOString().slice(0,10); labels.push(d.toLocaleDateString(undefined,{month:'short',day:'numeric'})); activity.push(state.history.filter(h => h.at.slice(0,10) === key).length); }
...
function exportData() { const data = { app: 'Confessio', version: 1, progress: state.progress, selected: [...state.selected], history: state.history, exportedAt: new Date().toISOString() }; const url = URL.createObjectURL(new Blob([JSON.stringify(data, null, 2)], {type:'application/json'})); const a = document.createElement('a'); a.href = url; a.download = `confessio-backup-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url); showToast('Backup downloaded.'); }
async function importData(file) { try { const data = JSON.parse(await file.text()); if (data.app !== 'Confessio' || !data.progress || !Array.isArray(data.history)) throw new Error(); state.progress = data.progress; state.selected = new Set(data.selected || []); state.history = data.history; await persist(); renderLibrary(); showToast('Backup restored.'); } catch { showToast('That file is not a valid Confessio backup.'); } }
...
async function init() {
  try { const [documentData, saved] = await Promise.all([fetch(DATA_URL).then(r => { if (!r.ok) throw new Error('data'); return r.json(); }), dbGet('userData')]); state.questions = documentData.questions; if (saved) { state.progress = saved.progress || {}; state.selected = new Set(saved.selected || []); state.history = saved.history || []; } renderLibrary(); }
  ...
  $('select-visible').onclick = () => { visibleQuestions().forEach(q => state.selected.add(q.number)); persist(); renderLibrary(); }; $('clear-selection').onclick = () => { state.selected.clear(); persist(); renderLibrary(); };
  $('select-all-checkbox').onchange = e => { visibleQuestions().forEach(q => e.target.checked ? state.selected.add(q.number) : state.selected.delete(q.number)); persist(); renderLibrary(); };
